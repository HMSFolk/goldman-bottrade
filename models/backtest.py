...
# ✅ FIX CRITICAL-2: get_config() แทน yaml.safe_load โดยตรง
from config import get_config
CFG = get_config()

log = logging.getLogger("models")

# ✅ FIX CRITICAL-3+4: absolute paths + ไม่ mkdir ที่ module-level
PROCESSED_DIR = _ROOT / CFG['paths']['data_processed']
MODELS_DIR    = _ROOT / CFG['paths']['models_saved']
REPORTS_DIR   = _ROOT / CFG['paths']['reports']

# ✅ FIX HIGH-6: pandas 2.0 deprecated "1T","15T","1H" → "1min","15min","1h"
TF_FREQ = {
...
class WalkForwardResult:
    ...
    def mean_sharpe(self) -> float:
        # ✅ FIX HIGH-7: float('nan') != float('nan') เสมอเป็น True (NaN ≠ NaN)
        #    ต้องใช้ np.isnan() แทน
        sharpes = [f.sharpe_ratio for f in self.folds
                   if not np.isnan(f.sharpe_ratio)]
    ...
    if method == "ensemble":
        ...
        for i in range(100, len(df)):     # warmup 100 bars
            signal = ensemble.predict(
                df.iloc[:i+1], method="soft"   # ensemble ต้องการ context
            )
        ...
        # ✅ FIX HIGH-5: df.iloc[[i]] แทน df.iloc[:i+1] — O(n) ไม่ใช่ O(n²)
        #    XGB/LGBM ใช้แค่ features ของ row นั้น ไม่ต้องการ history
        for i in range(50, len(df)):
            r = predict(payload, df.iloc[[i]], min_conf=min_conf)
        ...
        payload = load_model(symbol)
        for i in range(50, len(df)):
            r = predict(payload, df.iloc[[i]], min_conf=min_conf)   # ✅ FIX HIGH-5
            signals_dir.iloc[i] = r['direction']

    elif method == "rule":
        from models.rule_based import RuleBasedStrategy
        strategy = RuleBasedStrategy()
        # ✅ FIX HIGH-5: ใช้ apply แทน iterrows — เร็วกว่า + ไม่ SettingWithCopyWarning
        signals_dir = df.apply(
            lambda row: strategy.generate_signal(row).direction,
            axis=1,
        )
...
def run_backtest(
    ...
    """
    freq = TF_FREQ.get(timeframe, "15min")  # ✅ FIX HIGH-6: default ก็ต้องใช้ pandas 2.0 format
    price= df['close']
...
def run_deploy_checklist(
    ...
    # ── ตรวจ 7: .env มีค่าครบ ─────────────────────────────────
    # ✅ FIX MEDIUM-8: ใช้ get_config() แทน load_dotenv()+os.getenv
    _cfg    = get_config()
    env_ok  = all([
        _cfg['mt5'].get('login'),
        _cfg['mt5'].get('password'),
        _cfg['mt5'].get('server'),
    ])
    results['env_config'] = {
        'pass'  : env_ok,
        'detail': {
            'MT5_LOGIN'   : bool(_cfg['mt5'].get('login')),
            'MT5_PASSWORD': bool(_cfg['mt5'].get('password')),
            'MT5_SERVER'  : bool(_cfg['mt5'].get('server')),
        },
        'note'  : ".env ต้องมีครบ",
    }
...
if __name__ == "__main__":
    ...
    for sym in args.symbols:
        # ✅ สร้าง reports dir เมื่อจำเป็น (ไม่ใช่ module-level)
        ...
        if args.checklist:
            ...
            # ✅ FIX MEDIUM-9: run_backtest คืน BacktestMetrics เดียว
