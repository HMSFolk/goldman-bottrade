...
# ✅ FIX CRITICAL-2: ย้าย torch ออกจาก module-level
#    torch (~2GB) เป็น optional — ถ้าไม่ install จะ crash ทั้งไฟล์
#    ใช้ lazy import ใน function แทน

# ✅ FIX HIGH-1: ใช้ get_config() แทน hardcoded relative path
from config import get_config
_CFG = get_config()

log = logging.getLogger("models")

# ✅ FIX MEDIUM-4: absolute paths จาก project root
MODELS_DIR = _ROOT / _CFG['paths']['models_saved']
BACKUP_DIR = _ROOT / _CFG['paths']['models_saved'] / ".." / "backup"
...
def list_models() -> list[dict]:
    ...
    for path in sorted(MODELS_DIR.glob("*.pth")):
        try:
            # ✅ FIX CRITICAL-2: lazy import torch
            # ✅ FIX MEDIUM-3: weights_only=False สำหรับ dict payload, PyTorch 2.4+ ต้องระบุ
            import torch
            payload = torch.load(
                path,
                map_location='cpu',
                weights_only=False,   # เราโหลด dict ที่มี meta ด้วย
            )
            meta    = payload.get('meta', {})
...
def cleanup_old_backups(keep_last: int = 5):
    ...
    # ✅ FIX MEDIUM-5: filter เฉพาะ directory ไม่นับ file (เช่น .gitkeep)
    backups = sorted(
