...
# ✅ FIX BUG-1+2: ใช้ get_config() แทน open(config.yaml) + ลบ yaml import
...
class Signal:
    ...
    def is_actionable(self) -> bool:
        ...
        # ✅ FIX BUG-3: อ่าน min_confidence จาก config ไม่ hardcode
        min_conf = get_config()['signal']['min_confidence']
        return self.direction != 0 and self.confidence >= min_conf
...
class RuleBasedStrategy:
        ...
        if _valid(adx):
            if adx >= self.adx_min:
                if _valid(adx_pos, adx_neg):
                    if adx_pos > adx_neg:
                        buy_score += 1.0
                        reasons.append(f"ADX={adx:.0f} +DI>{adx_pos:.0f}(+1)")  # ✅ FIX BUG-4
                    else:
                        sell_score += 1.0
                else:
    ...
    def generate_signals(
        ...
        ✅ FIX BUG-5: ใช้ df.apply() แทน iterrows() — เร็วกว่า ~30%
        ✅ FIX BUG-6: return_objects=True คืน Signal objects
                       เพื่อให้ backtest ไม่ต้อง call generate_signal ซ้ำ
        """
        if return_objects:
            sigs = [self.generate_signal(row) for _, row in df.iterrows()]
            return pd.Series(sigs, index=df.index, name='signal_obj')
        # direction เท่านั้น — เร็วกว่าด้วย apply
        return df.apply(
            lambda row: self.generate_signal(row).direction,
            axis=1,
        ).rename('signal')
    ...
    def backtest(
        ...
        # ✅ FIX BUG-6: ใช้ Signal objects โดยตรง ไม่ต้อง call generate_signal อีกรอบ
        sig_objects = self.generate_signals(df, return_objects=True)

        for i, (idx, row) in enumerate(df.iterrows()):
            price   = row['close']
            sig_obj = sig_objects.iloc[i]
            signal  = sig_obj.direction
            ...
            # ✅ FIX BUG-6: ใช้ sig_obj จาก loop แทนการ call generate_signal ซ้ำ
            if position == 0 and signal != 0:
                if sig_obj.confidence >= get_config()['signal']['min_confidence']:
                    position    = signal
                    entry_price = price
