...
def _add_ichimoku(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Tenkan-sen (9)
    df['ichi_conv']  = (h.rolling(9).max()  + l.rolling(9).min())  / 2

    # Kijun-sen (26)
    df['ichi_base']  = (h.rolling(26).max() + l.rolling(26).min()) / 2
    ...
    df['ichi_b']     = ((h.rolling(52).max() + l.rolling(52).min()) / 2).shift(26)
...
def _add_supertrend(df: pd.DataFrame,
    ...
    # คำนวณ Supertrend (iterative — ต้องทำ loop)
    st    = pd.Series(np.nan, index=df.index)
    trend = pd.Series(1, index=df.index)

    for i in range(1, len(df)):
        # Upper band
        if upper.iloc[i] < upper.iloc[i-1] or c.iloc[i-1] > upper.iloc[i-1]:
            upper.iloc[i] = upper.iloc[i]
        else:
            upper.iloc[i] = upper.iloc[i-1]

        # Lower band
        if lower.iloc[i] > lower.iloc[i-1] or c.iloc[i-1] < lower.iloc[i-1]:
